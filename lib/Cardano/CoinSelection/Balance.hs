...
deriving instance
    (SelectionContext ctx) => Eq (SelectionBalanceError ctx)
deriving instance
    (SelectionContext ctx) => Show (SelectionBalanceError ctx)
    ...
    loop !adaRequired !pairsNonEmpty = case pairsNonEmpty of
        ...
        (m, _) :| (p : ps)
            | TokenMap.isEmpty m && adaAvailable < adaRequired ->
                ...
                let adaRequired' = adaRequired <\> minCoinFor m
...
splitBundlesWithExcessiveTokenQuantities bs maxQuantity =
    (`TokenBundle.equipartitionQuantitiesWithUpperBound` maxQuantity)
        =<< bs
...
runRoundRobin state demote processors =
    runIdentity $
        runRoundRobinM state demote $
            fmap Identity <$> processors

runRoundRobinM
    :: (Monad m) => s -> (s' -> s) -> [(s -> m (Maybe s'))] -> m s
...
selectedAssetQuantity
    :: (IsUTxOSelection s u) => AssetId -> s u -> Natural
