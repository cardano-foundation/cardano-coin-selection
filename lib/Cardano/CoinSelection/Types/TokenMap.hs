...
import Cardano.CoinSelection.Internal.Numeric
...
import qualified Cardano.CoinSelection.Types.TokenQuantity as TokenQuantity
...
{- | Partitions a token map into a given number of parts, splitting assets
  as evenly as possible across the parts.
-}
equipartitionAssets
    :: TokenMap
    -> NonEmpty a
  ...
  where
    ...
    assetCounts =
        fromIntegral @Natural @Int
            <$> equipartitionNatural
                (fromIntegral @Int @Natural assetCount)
                mapCount

    generateChunk
        :: (NonEmpty Int, [aq])
        -> ([aq], Maybe (NonEmpty Int, [aq]))
...
{- | Partitions a token map so that no token quantity exceeds the given
  upper bound.
-}
equipartitionQuantitiesWithUpperBound
    :: TokenMap
...
{- | Internal helper: partitions all quantities in a token map into a given
