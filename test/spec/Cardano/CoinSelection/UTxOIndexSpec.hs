...
import Cardano.CoinSelection.Test.Laws
    ( testLawsMany
    )
...
prop_shrink_invariant
    :: (u ~ Size 4 TestUTxO) => UTxOIndex u -> Property
...
prop_insert_balance u b i =
  ...
  where
    expected =
        b <> case UTxOIndex.lookup u i of
            Nothing ->
                UTxOIndex.balance i
            Just b' ->
                UTxOIndex.balance i <\> b'
...
-}
prop_selectRandomSetMember_coversRangeUniformly
